...
async function seed() {
    ...
    .values([
      {
        ...
        content:
          'Pemberitahuan kepada seluruh member, camp akan tutup pada tanggal 1 Mei untuk libur nasional Hari Buruh. Latihan akan kembali normal pada 2 Mei.',
        type: 'Info',
        isActive: true,
      },
      {
        ...
        content:
          'Dapatkan tambahan 4 sesi gratis untuk setiap perpanjangan paket Dewasa selama bulan Ramadhan. Promo berlaku hingga akhir bulan.',
        type: 'Promo',
        isActive: true,
      },
    ])
  ...
  // ─── 5. Seed Sample Member Profiles ───────────────────────
  console.log('  → Inserting sample member profiles...');
  const dewasa8x = insertedPackages.find(
    (p) => p.category === 'Dewasa' && p.name === '8x Sebulan',
  );
  const pelajar4x = insertedPackages.find(
    (p) => p.category === 'Pelajar' && p.name === '4x Sebulan',
  );
  const dewasaVisit = insertedPackages.find(
    (p) => p.category === 'Dewasa' && p.name === '1x Visit',
  );
  const dewasaLadies = insertedPackages.find(
    (p) => p.category === 'Dewasa' && p.name === '8x Ladies',
  );
